...
self.color = color
self.wheels = wheels
print("--- Vehicle **init** executed ---")

class Car(Vehicle):
def **init**(self, color, wheels, model): # 1. Call the parent's constructor using super() # This initializes 'color' and 'wheels' attributes
...
class Truck(Vehicle):
def drive(self): # 1. Execute the parent's drive logic first
super().drive()
